 ...
 */
public class EliminaPoblacionesNoPreparadaApp {
  ...
  private static int eliminaPorCodigo(String codigo) throws SQLException {
    GeneradorSentencia generador = new GeneradorSentenciaCodigo(codigo);
    return eliminaConGenerador(generador);
  }

  private static int eliminaPorExtension(double extension) throws SQLException{
    GeneradorSentencia generador = new GeneradorSentenciaExtensionMenorQue(extension);
    return eliminaConGenerador(generador);
  }

  private static int eliminaPorPoblacion(int poblacion) throws SQLException {
    GeneradorSentencia generador = new GeneradorSentenciaPoblacionMayorQue(poblacion);
    return eliminaConGenerador(generador);
  }


  private static int eliminaConGenerador(GeneradorSentencia generador) throws SQLException {
    ...
    try (Connection conexion = DriverManager.getConnection(URL_DB);
      ...
      int resultado = sentencia.executeUpdate(generador.generaSentencia());
